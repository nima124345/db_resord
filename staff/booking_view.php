...
<br>
    <font face="TH Sarabun New" size="5">ฟอร์มยกเลิกการจองห้องพัก</font>
    <hr> 
    <form action="bookingStatusUpdate.php" method="post">
        <div class="row mb-2">
         <label><font face="TH Sarabun New" size="4">เลือกสถานะ</font></label>
         <select name="booking_status" class="form-control" required>
           <option value="0">ยกเลิกการจอง</option>
         </select>
        </div>
        <div class="row mb-2">
         <label><font face="TH Sarabun New" size="4">ระบุเหตุผลที่ยกเลิก</font></label>
          <textarea name="remark" class="form-control" required placeholder="เช่น ไม่ชำระเงิน เป็นต้น"></textarea>
        </div>
         <div class="row">
          <input type="hidden" name="booking_id" value="<?=$rwbD['booking_id'];?>">
          <button type="submit" name="act" value="cancel" class="btn btn-primary">บันทึกการยกเลิก</button>
        </div>
    </form>
  </div>
</div>