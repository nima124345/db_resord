...
<br>
    <font face="TH Sarabun New" size="5">ฟอร์ม Check-IN || บันทึกการเข้าพัก </font>
    <hr> 
    <form action="bookingStatusUpdate.php" method="post">
         <div class="row mb-2">
         <label><font face="TH Sarabun New" size="4">ห้องพัก (พนง.จองให้)</font></label>
          <input type="text" class="form-control" disabled value="<?=$rwbD['room_number'];?>">
        </div>
        <div class="row mb-2">
         <label><font face="TH Sarabun New" size="4">ทะเบียนรถ (ถ้ามี) </font></label>
          <input type="text" name="car_no" class="form-control" placeholder="ไม่บังคับกรอกข้อมูล">
        </div>
         <div class="row">
          <input type="hidden" name="booking_id" value="<?=$rwbD['booking_id'];?>">
          <button type="submit" name="act" value="ChkIN" class="btn btn-primary">บันทึกการเข้าพัก</button>
        </div>
    </form>
  </div>
</div>